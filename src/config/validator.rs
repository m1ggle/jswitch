...
mod tests {
    ...
    fn sources_round_trip_through_get_set() {
        ...
        config
            .set(
                "sources.adoptopenjdk",
                "https://mirror.example.com/adoptium".to_owned(),
            )
            .unwrap();

        assert_eq!(
            config.get("sources.adoptopenjdk").unwrap(),
            Some("https://mirror.example.com/adoptium".to_owned())
        );
    }
    ...
    fn entries_include_sources_keys() {
        let mut config = Config::default();
        config.sources.adoptopenjdk = Some("https://mirror.example.com/adoptium".to_owned());
        ...
        assert!(
            entries
                .iter()
                .any(|(key, value)| key == "sources.adoptopenjdk"
                    && value == "https://mirror.example.com/adoptium")
        );
    }
}
